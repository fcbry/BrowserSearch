...
namespace Community.Powertoys.Run.Plugin.BrowserSearch
{
  public class Main : IPlugin, ISettingProvider, IReloadable
  {
    ...
    public IEnumerable<PluginAdditionalOption> AdditionalOptions => new List<
        PluginAdditionalOption>() {
    new() { Key = MaxResults, DisplayLabel = "Maximum number of results",
            DisplayDescription = "Maximum number of results to show. Set to " +
                                 "-1 to show all (may decrease performance)",
            PluginOptionType =
                PluginAdditionalOption.AdditionalOptionType.Numberbox,
            NumberValue = 8 },
    new() {
      Key = SingleProfile, DisplayLabel = "Browser profile",
      DisplayDescription =
          "The name of the browser profile whose history will be loaded.\n" +
    ...
    {
      _maxResults = (int)(settings?.AdditionalOptions
                              ?.FirstOrDefault(x => x.Key == MaxResults)
                              ?.NumberValue ??
                          15);

      PluginAdditionalOption? profile =
    ...
    private void InitDefaultBrowser()
    {
      ...
      {
        Log.Error("Couldn't retrieve default browser name: Timeout",
                  typeof(Main));
        return;
      }
      ...
      switch (BrowserInfo.Name)
      {
        case "Arc":
          _defaultBrowser = new Chromium(
              Path.Join(localappdata,
                        @"Packages\TheBrowserCompany.Arc_" +
                        @"ttt1ap7aakyb4\LocalCache\Local\Arc\User Data"),
        ...
        case "Brave Beta":
          _defaultBrowser =
              new Chromium(Path.Join(localappdata,
        ...
        case "Google Chrome":
          _defaultBrowser =
              new Chromium(Path.Join(localappdata, @"Google\Chrome\User Data"),
                           _selectedProfileName);
          break;
        case "Google Chrome Beta":
          _defaultBrowser =
              new Chromium(Path.Join(localappdata, @"Google\Chrome Beta\User Data"),
                           _selectedProfileName);
          break;
        case "Google Chrome Dev":
          _defaultBrowser =
              new Chromium(Path.Join(localappdata, @"Google\Chrome Dev\User Data"),
                           _selectedProfileName);
          break;
        case "Google Chrome Canary":
          _defaultBrowser =
              new Chromium(Path.Join(localappdata, @"Google\Chrome SxS\User Data"),
                           _selectedProfileName);
          break;
        case "Microsoft Edge":
          _defaultBrowser =
              new Chromium(Path.Join(localappdata, @"Microsoft\Edge\User Data"),
                           _selectedProfileName);
          break;
        ...
        case "Microsoft Edge Dev":
          _defaultBrowser =
              new Chromium(Path.Join(localappdata, @"Microsoft\Edge Dev\User Data"),
                           _selectedProfileName);
          break;
        case "Microsoft Edge Canary":
          _defaultBrowser =
              new Chromium(Path.Join(localappdata, @"Microsoft\Edge SxS\User Data"),
                           _selectedProfileName);
          break;
        case "Thorium":
          _defaultBrowser = new Chromium(
        ...
        case "Wavebox":
          _defaultBrowser =
              new Chromium(Path.Join(localappdata, @"WaveboxApp\User Data"),
                           _selectedProfileName);
          break;
        default:
          Log.Error(
              $"Unsupported/unrecognized default browser '{BrowserInfo.Name}'",
              typeof(Main));
          MessageBox.Show($"Browser '{BrowserInfo.Name}' is not supported",
                          "BrowserSearch");
          return;
      }
    ...
    public List<Result> Query(Query query)
    {
      ...
      {
        // Returning the whole history here makes the search lag, so return only
        // some entries
      ...
      if (_maxResults != -1)
      {
        // Rendering the UI of every search entry is slow, so only show top
        // results
    ...
    {
      // Since PT Run's FuzzySearch is too slow, and the history usually has a lot
      // of entries, lets calculate the scores manually using a faster (but less
      // accurate) method
      float titleScore =
          title.Contains(query, StringComparison.InvariantCultureIgnoreCase)
